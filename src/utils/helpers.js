...
export const isSlotAvailable = (timeSlot, appointments, barberId, date, serviceDuration = 30, services = []) => {
    const slotStart = parseTime(timeSlot)
    const slotEnd = slotStart + serviceDuration * 60 * 1000

    return !appointments.some(apt => {
        if (apt.barberId !== barberId || apt.date !== date || apt.status === 'cancelled') return false
        ...
        const aptEnd = aptStart + existingDuration * 60 * 1000
